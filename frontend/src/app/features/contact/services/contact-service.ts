...
export const contactService = {
  register: async (contact: Contact): Promise<Contact> => {
    const { data } = await httpClient.post(API_ENDPOINTS.CONTACT.REGISTER, contact);
    return data;
  },

  search: async (params: ContactSearchParams): Promise<Contact[]> => {
    const { data } = await httpClient.get(API_ENDPOINTS.CONTACT.SEARCH, { params });
    return data;
  },

  update: async (id: number, contact: Contact): Promise<Contact> => {
    const { data } = await httpClient.put(`${API_ENDPOINTS.CONTACT.REGISTER}/${id}`, contact);
    return data;
  },
